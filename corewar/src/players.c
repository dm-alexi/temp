...
/*   Updated: 2020/04/05 17:12:21 by sscarecr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_players(t_vm *vm, char **files)
{
	...
	while (i < vm->num_players)
	{
		...
		if (!(vm->players[i].code = (t_byte*)malloc(vm->players[i].header.prog_size)))
			sys_error(NULL);
		if ((r = read(fd, (vm->players + i)->code,
		vm->players[i].header.prog_size)) < (int)vm->players[i].header.prog_size)
