...
void	wait(long long finish)
{
	long long	left;

	while ((left = finish - now()) > 10 * STEP)
		usleep(10 * STEP);
	if (left > 0)
		usleep(left);
}
...
void	*philos(void *param)
{
	...
	while (game->number > 1 && !game->stop)
	{
		...
		if (game->max_meals && (++(p->meals) == game->max_meals))
			++(game->finished);
		pthread_mutex_unlock(&game->output);
		wait(p->last_meal + game->tte + game->tts);
	}
	return (NULL);
}
...
int	main(int ac, char **av)
{
	...
	game = (t_game){.finished = 0, .number = arg[0], .ttd = arg[1] * 1000LL,
		...
		.forks = NULL, .philos = NULL, .threads = NULL, .stop = FALSE};
